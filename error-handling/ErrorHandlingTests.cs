...
namespace Exercism.CSharp.Solutions.ErrorHandlingExercise.Tests
{
    public class ErrorHandlingTests
    {
        ...
        {
            var successfulResult = ErrorHandling.HandleErrorWithOutParam("1", out int result);
            Assert.True(successfulResult);
            Assert.Equal(1, result);

            var failureResult = ErrorHandling.HandleErrorWithOutParam("a", out _);
